package com.example.gradleTest2.controll;
...
public class BbsController {
	...
	public String write(Model model) {
		...
		model.addAttribute("simpleBbsDto", new SimpleBbsDto()); // 유효성 검사를 해야 하기 때문 확인하기 위해서 검사를 진행 안할려면 return 만 하면 된다
	...
	// 글작성 저장
	@PostMapping("/write")
	public String write(@Valid SimpleBbsDto simpleBbsDto, BindingResult  bind, Model model) {
		
		if(bind.hasErrors()) { // 작성한 값의 유효성검사를 통과하지못하면 작성페이지로 이동
			return "write";
		}
	...
	// 글수정 저장
	@PostMapping("/update")
	public String update(@Valid SimpleBbsDto simpleBbsDto, BindingResult bind, Model model){
...
/*
	...
	4페이지 제작
	글쓰기 양식 과 권환 - 제목, 작성자, 내용, 비밀번호 
					모든 접속 유저
	수정 양식 과 권환 - 제목, 내용, 
					작성자 비밀번호
	삭제 권환 - 작성자
	글보기 양식 과 권환 - 작성자, 제목, 내용
					모든 유저
	...
	프로그램 설계 - 스프링( controller -> DAO(DTO) )
			  ...
			  - 데이터베이스 - mybatis
			  	...
			  	BbsController
			  		...
			  		저장 : /write(POST) - public String write(@Vaild SimpleBbsDto simpleBbsDto, BindingResult bind, Model model) 메서드
			  		글보기 : /view(GET) - public String view(@RequestParam String id, Model model) 메서드
			  		글수정 : /updateForm(POST) - public String updateForm(@RequestParam String id, Model model) 메서드
			  		저장 : /update(POST) - public String update(@Vaild SimpleBbsDto simpleBbsDto, BindingResult bind, Model model) 메서드
	...
	고객 요청 -> 추가질문 -> 견적서 작성만큼만 작업
	미팅 - 개발자 ( 팀장 -> 개발부서에서 선임 )
	당근 같은 사이트 개발
	로그인, 회원가입 - 회원가입 양식, 본인인증-통신사
	
	게시판 - 작성자 (로그인) , 제목x, 내용o -> 이미지, 표, 링크
			..........
